...
//		);
		...
		if ($category_info) {
			...
			if ($category_info['image']) {
				...
				$data['thumb'] = $this->model_tool_image->resize($category_info['image'], 1260, 400 );
			} else {
				$data['thumb'] = '';
			}
			...
			foreach ($results as $result) {
				if ($result['image']) {
                    $image = $this->model_tool_image->resize($result['image'], 1260, 400);
                } else {
                    $image = $this->model_tool_image->resize('placeholder.png', 1260, 400);
                }
            ...
            $blog_category_child = $this->model_blog_category->getCategories($category_info["blog_category_id"]);

            $data['description'] = html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8');

            if(count($blog_category_child) > 0){

                foreach ($blog_category_child as $blog_cat) {
                  //  var_dump($blog_cat);die;

                    $article_data = array(
                        'filter_category_id' => $blog_cat['blog_category_id'],
                    );
                    ...
                    foreach ($results as $r) {
                        ...
                        $blog_articles[] = [
                            'title' => $r['name'],
                            'href' => $this->url->link('blog/article', 'blog_category_id=' . $blog_cat['blog_category_id'] . '&article_id=' . $r['article_id']),

                        ];

                     //   var_dump($blog_articles);die;
                    }

                    if ($blog_cat['image']) {
                        $image = $this->model_tool_image->resize($blog_cat['image'], 300, 200);
                    } else {
                        $image = $this->model_tool_image->resize('placeholder.png', 300, 200);
                    }

                    $data['blog_category_child'][] = array(
                        'name' => $blog_cat['name'] ,
                        'href'  => $this->url->link('blog/category', 'blog_category_id=' . $this->request->get['blog_category_id'] . '_' . $blog_cat['blog_category_id'] . $url),
                        'image' => $image,
                        'articles'=> $blog_articles,
                        'description' => utf8_substr(strip_tags(html_entity_decode($blog_cat['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('configblog_article_description_length')) . '..',
                ...
                $this->response->setOutput($this->load->view('blog/category_child', $data));
            }
            else {
                $this->response->setOutput($this->load->view('blog/category', $data));
            }
